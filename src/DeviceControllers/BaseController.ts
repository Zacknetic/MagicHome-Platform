...
export class BaseController {
      ...
      switch (deviceWriteStatus) {
        case ready:
          ...
          */
          ...
          await this.writeStateToDevice(this.newColorCommand, commandOptions).then(successMsg => {
  ...
  private async writeStateToDevice(deviceCommand: IDeviceCommand, commandOptions: ICommandOptions): Promise<string> {
    ...
    return new Promise<string>(async (resolve, reject) => {
      ...
      await this.prepareColorCommand(sanitizedDeviceCommand);
      const isValidState = await this.testValidState(sanitizedDeviceCommand);
      if (isValidState) {
        this.overwriteLocalState(sanitizedDeviceCommand);
        resolve('Successful state write')
      } else {
        reject(`Unsuccessful state write:`);
      }
  ...
  private async testValidState(deviceCommand: IDeviceCommand){
    return new Promise(async (resolve, reject) => 
      setTimeout(async () => {
        return await this.fetchState().then((deviceState) => {
  ...
  private async prepareColorCommand(deviceCommand: IDeviceCommand): Promise<string> {
    ...
    return new Promise(async (resolve) => {
      if (this.devicePowerCommand) {
        ...
        if (this.deviceAPI.needsPowerCommand && deviceCommand.isOn) {
