...
export class AnimationManager {
    ...
    constructor(controllers: BaseController[], animationBlueprints: IAnimationBlueprint[], STEP_INTERVAL_MS: number = 20) {
    ...
    public generateAnimationLoopFromBlueprint(animationBlueprint: IAnimationBlueprint) {
        ...
        const animation = new AnimationLoop(animationBlueprint, assignedLightsNames, this.STEP_INTERVAL_MS);
        this.animations.push(animation);
    }

    public addLightToAnimationLoop(controller: BaseController, animation: AnimationLoop) {
        console.log('adding light to animation loop' + controller.id)
    ...
    private tick() {
        ...
        if (elapsedTime >= this.STEP_INTERVAL_MS) {
            this.startTime = process.hrtime()[0] * 1000 + process.hrtime()[1] / 1e6; //reset startTime to current time
            setTimeout(this.tick.bind(this), this.STEP_INTERVAL_MS);
        } else {
            setTimeout(this.tick.bind(this), this.STEP_INTERVAL_MS - elapsedTime);
        }
    }
    ...
    private tickLights() {
        this.lightMap.forEach(async (light) => {
            ...
            const nextAnimation = this.animations.find(animation => light.controller.hasAnimation(animation.name) && animation.getAnimationStep(light.id, this.STEP_INTERVAL_MS) !== undefined && Math.max(...Object.values(animation.getAnimationStep(light.id, this.STEP_INTERVAL_MS))) > 0);
            let currentStep;
            if (!nextAnimation) currentStep = { red: 100, green: 0, blue: 0, warmWhite: 0, coldWhite: 0 };
            else currentStep = nextAnimation.getAnimationStep(light.id, this.STEP_INTERVAL_MS);
