...
const { POWER_COMMAND, COLOR_COMMAND, ANIMATION_FRAME, QUERY_COMMAND } =
	COMMAND_TYPE;
...
export class BaseController {
	...
	constructor(completeDevice: ICompleteDevice) {
		...
		this.deviceState = mergeDeep(
			{},
			completeDevice.completeResponse.deviceState
		);
	...
	public async setOn(value: boolean) {
		...
		const deviceCommand: IDeviceCommand = mergeDeep({}, DEFAULT_COMMAND, {
			isOn: value,
		});
		const commandOptions = mergeDeep({}, DEFAULT_COMMAND_OPTIONS, {
	...
	private async writeCommand(
		...
		const newDeviceCommand = adjustCommandToAPI(
			deviceCommand,
			commandOptions,
			this.deviceAPI
		);
		...
		const result = (await this.deviceInterface.sendCommand(
			newDeviceCommand,
			commandOptions
		)) as ICompleteResponse;
		if (result?.deviceState) this.overwriteLocalState(result.deviceState);
		return result;
	}
	...
	public setLightColor(color: IAnimationColorStep) {
		...
		const commandOptions: ICommandOptions = {
			...
			timeoutMS: 50,
			isEightByteProtocol:
				this.getCachedDeviceInformation().deviceAPI.isEightByteProtocol,
			colorAssist: true,
		};
	...
	private async precheckPowerState(
		...
		if (
			this.first &&
			this.deviceAPI.needsPowerCommand &&
			!this.deviceState.isOn &&
			deviceCommand.isOn
		) {
	...
	public async fetchStateRGB(timeout: number = 500): Promise<IDeviceState> {
		...
		if (typeof completeResponse == 'undefined')
