import {
  ...
  combineDeep,
  cloneDeep,
} from "magichome-core/dist/utils/miscUtils";
...
export function getAPI(deviceMetaData: DeviceMetaData) {
  ...
  if (!deviceTypesMap.has(controllerHardwareVersion))
    throw new NoMatchingAPIError(deviceMetaData);
  const deviceAPI: DeviceAPI = deviceTypesMap.get(
...
function adjustCCT(
  ...
  const cwAdj = Math.round(coldWhite / 2),
    wwRedAdj = Math.round(warmWhite / 2),
    wwGreenAdj = Math.round(warmWhite / 6.8),
    wwBlueAdj = Math.round(warmWhite / 28.4);
  //todo handle non RGB devices, single brightness devices

  // simultaneousCCT, 5-colors, ColorMask set to CCT
  if (byteOrder.length == 5 && simultaneousCCT && colorMask == ColorMask.CCT) {
    /*do nothing for now */
  }
  ...
  // (adjusts for the lack of simultaneous CCT and RGB by adding adjustments calculated from both warmWhite and coldWhite to the RGB channels)
    ...
    newDeviceCommand = combineDeep<DeviceCommand>(deviceCommand, {
      RGB: {
        ...
        blue: clamp(blue + cwAdj + wwBlueAdj, 0, 255),
      },
